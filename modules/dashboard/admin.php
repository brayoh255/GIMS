...
// Dashboard logic with proper error handling
try {
    // Total Users
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM users");
    $totalUsers = $stmt->fetchColumn();
    
    // Total Stock - verify your cylinder_types table has current_stock column
    $stmt = $pdo->query("SELECT IFNULL(SUM(current_stock), 0) as total FROM cylinder_types");
    $totalStock = $stmt->fetchColumn();
    
    // Total Sales - verify your sales table has total_amount and created_at columns
    $stmt = $pdo->prepare("SELECT IFNULL(SUM(total_amount), 0) as total FROM sales WHERE MONTH(created_at) = ? AND YEAR(created_at) = ?");
    $stmt->execute([$currentMonth, $currentYear]);
    $totalSales = $stmt->fetchColumn();
    
    // Total Expenses - verify your expenses table has amount and created_at columns
    $stmt = $pdo->prepare("SELECT IFNULL(SUM(amount), 0) as total FROM expenses WHERE MONTH(created_at) = ? AND YEAR(created_at) = ?");
    $stmt->execute([$currentMonth, $currentYear]);
    $totalExpenses = $stmt->fetchColumn();
    
    // Total Debts - verify your debts table has balance and created_at columns
    $stmt = $pdo->prepare("SELECT IFNULL(SUM(balance), 0) as total FROM debts WHERE balance > 0 AND MONTH(created_at) = ? AND YEAR(created_at) = ?");
    $stmt->execute([$currentMonth, $currentYear]);
...
<body>
    <!-- [Previous sidebar and top nav content remains the same] -->
         <!-- Sidebar -->
                ...
                <div class="col-md-6 col-lg-3">
                    <div class="stat-card" id="statCard2">
                        <div class="card-body">
                            ...
                            <h3 class="card-value"><?= number_format($totalExpenses, 2) ?> Tsh</h>
                            <div class="card-footer">
                ...
                <div class="col-md-6 col-lg-3">
                    <div class="stat-card" id="statCard3">
                        <div class="card-body">
                            ...
                            <h3 class="card-value" id="totalSales"><?= number_format($totalSales, 2) ?> Tsh</h3>
                            <div class="card-footer">
                ...
                <div class="col-md-6 col-lg-3">
                    <div class="stat-card" id="statCard4">
                        <div class="card-body">
                            ...
                            <h3 class="card-value" id="totalDebts"><?= number_format($totalDebts, 2) ?> Tsh</h3>
                            <div class="card-footer">
